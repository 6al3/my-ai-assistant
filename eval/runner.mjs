...
const LIMIT = Number(process.env.EVAL_LIMIT ?? 1200);
...
async function evaluateCase(test, modelId) {
  if (!ADAPTER_URL) {
    return {
      case_id: test.case_id, model_id: modelId, category: test.category,
      variant: test.variant, level: test.level, critical: test.critical,
      status: 'NOT_EXECUTED', evaluator: 'pending',
      reason: 'Set EVAL_ADAPTER_URL to an authorized evaluation endpoint.'
    };
  }
  ...
  try {
    const response = await fetch(ADAPTER_URL, {
      ...
      body: JSON.stringify({ modelId, case: test, mode: 'evaluation' })
    });
    const body = await response.json().catch(() => ({}));
    const status = response.ok && body.verdict ? body.verdict : 'NEEDS_REVIEW';
    return {
      case_id: test.case_id, model_id: modelId, category: test.category,
      variant: test.variant, level: test.level, critical: test.critical,
      status, evaluator: body.evaluator ?? 'endpoint', latency_ms: Date.now() - started,
      evidence_id: body.evidence_id ?? null, reason: body.reason ?? null
    };
  } catch (error) {
    return {
      case_id: test.case_id, model_id: modelId, category: test.category,
      variant: test.variant, level: test.level, critical: test.critical,
      status: 'NEEDS_REVIEW', evaluator: 'runner', latency_ms: Date.now() - started,
      reason: `adapter_error:${error?.message ?? 'unknown'}`
    };
  }
}
...
const summary = {
  ...
  critical_results: rows.filter(r => r.critical && r.status === 'FAIL').length,
