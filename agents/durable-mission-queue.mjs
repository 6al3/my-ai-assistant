...
export class DurableMissionQueue {
  constructor({ filePath, maxAttempts = 3, leaseMs = 30_000, lockTimeoutMs = 5_000, lockRetryMs = 10, staleLockMs = 30_000, now = () => Date.now() } = {}) {
    ...
    if (!Number.isFinite(staleLockMs) || staleLockMs <= 0) throw new Error('staleLockMs must be > 0');
  ...
  async claim(worker) {
    ...
    if (worker.capabilities != null && !Array.isArray(worker.capabilities)) throw new Error('worker capabilities must be an array');
    return this.#mutate(state => {
  ...
  async requeueExpired() {
    return this.#mutate(state => this.#requeueExpiredInState(state));
  }
  ...
  async list({ status } = {}) {
    const state = this.#readState();
    return state.missions.filter(m => !status || m.status === status).map(m => structuredClone(m));
  }
  ...
  #readState() {
    ...
    for (const mission of state.missions) {
      ...
      if (!Array.isArray(mission.requiredCapabilities)) mission.requiredCapabilities = [];
      if (!Array.isArray(mission.dependsOn)) mission.dependsOn = [];
    }
    return state;
  }
