...
test('runs duration-bound calibration sequentially for every candidate before selecting winner', async () => {
  const seen = [];
  let active = 0;
  let peak = 0;
  const result = await runQubesCalibrationSelectionCommand({
    ...
    runCalibration: async config => {
      ...
      active -= 1;
      return { report: report(config.topologyId, config.topologyId === 'fast' ? 18 : 4) };
    },
...
test('validates complete candidate/config bijection before starting any Qubes calibration', async () => {
  ...
  await assert.rejects(
    () => runQubesCalibrationSelectionCommand(base, { runCalibration }),
    /calibration topology ids must be unique/
  );
  assert.equal(calls, 0, 'invalid calibration sets must fail before any synthetic workload starts');

  await assert.rejects(
    () => runQubesCalibrationSelectionCommand({
      ...base,
      calibrationConfigs: [
        { topologyId: 'fast', gitSha: SHA },
        { topologyId: 'unexpected', gitSha: SHA }
      ]
    }, { runCalibration }),
    /unexpected calibration topology unexpected/
  );
  ...
  await assert.rejects(
    () => runQubesCalibrationSelectionCommand({
      ...
      calibrationConfigs: [
        { topologyId: 'fast', gitSha: SHA },
        { topologyId: 'stable', gitSha: 'b'.repeat(40) }
      ]
    }, { runCalibration }),
    /calibration stable git SHA mismatch/
  );
  assert.equal(calls, 0);
});
