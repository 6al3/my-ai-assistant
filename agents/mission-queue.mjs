...
export class MissionQueue {
  ...
  enqueue({ task, priority = 0, requiredCapabilities = [], dependsOn = [], metadata = {}, idempotencyKey = null }) {
    ...
    const mission = { id: randomUUID(), task: task.trim(), priority, requiredCapabilities: [...new Set(requiredCapabilities)], dependsOn: dependencies, metadata, idempotencyKey, status: 'queued', attempts: 0, workerId: null, leaseToken: null, leaseUntil: null, createdAt: this.now(), updatedAt: this.now(), result: null, error: null };
  ...
  claim(worker) {
    ...
    mission.status='running'; mission.workerId=worker.id; mission.leaseToken=randomUUID(); mission.attempts+=1; mission.leaseUntil=this.now()+this.leaseMs; mission.updatedAt=this.now();
    return structuredClone(mission);
  }
  heartbeat(id, workerId, leaseToken = null) { const m=this.#ownedRunning(id,workerId,leaseToken); m.leaseUntil=this.now()+this.leaseMs; m.updatedAt=this.now(); return structuredClone(m); }
  complete(id, workerId, result=null, leaseToken = null) {
    const existing=this.missions.get(id);
    if(!existing)throw new Error('mission not found');
    if(existing.status==='completed'){
      if(existing.workerId!==workerId||!this.#tokenMatches(existing,leaseToken)||JSON.stringify(existing.result)!==JSON.stringify(result))throw new Error('mission completion conflicts with committed result');
      return structuredClone(existing);
    }
    const m=this.#ownedRunning(id,workerId,leaseToken); m.status='completed'; m.result=result; m.leaseUntil=null; m.updatedAt=this.now(); return structuredClone(m);
  }
  fail(id, workerId, error, leaseToken = null) { const m=this.#ownedRunning(id,workerId,leaseToken); m.error=String(error??'unknown failure'); m.workerId=null;m.leaseToken=null;m.leaseUntil=null;m.updatedAt=this.now();m.status=m.attempts>=this.maxAttempts?'failed':'queued';if(m.status==='failed')this.#propagateDependencyFailures();return structuredClone(m); }
  cancel(id, reason='cancelled') { const m=this.missions.get(id);if(!m)throw new Error('mission not found');if(TERMINAL.has(m.status))throw new Error(`mission is ${m.status}`);m.status='cancelled';m.error=String(reason);m.workerId=null;m.leaseToken=null;m.leaseUntil=null;m.updatedAt=this.now();this.#propagateDependencyFailures();return structuredClone(m); }
  requeueExpired() { const now=this.now();let terminalized=false;for(const m of this.missions.values()){if(m.status!=='running'||m.leaseUntil>now)continue;m.workerId=null;m.leaseToken=null;m.leaseUntil=null;m.updatedAt=now;m.error='worker lease expired';m.status=m.attempts>=this.maxAttempts?'failed':'queued';terminalized ||= m.status==='failed';}if(terminalized)this.#propagateDependencyFailures(); }
