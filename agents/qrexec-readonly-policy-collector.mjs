...
function normalizeInvocation(result, scenario) {
  ...
  if (!Number.isInteger(result.exitCode) || result.exitCode < 0 || result.exitCode > 255) {
    throw new Error(`scenario ${scenario}.exitCode must be an integer from 0 to 255`);
  }
  ...
  return {
    ...
    responseBytes: Number.isSafeInteger(result.responseBytes) && result.responseBytes >= 0 ? result.responseBytes : null,
    responseFrames: Number.isSafeInteger(result.responseFrames) && result.responseFrames >= 0 ? result.responseFrames : null
  };
}

export async function collectReadonlyQrexecPolicyQualification({
  gitSha,
  runtimeFingerprint,
  sourceQube,
  coordinatorQube,
  ...
  const attestationVerified = await verifyResponse(allowed.response);
  if (attestationVerified !== true) throw new Error('allowed response attestation verification failed');
  ...
  const report = buildReadonlyQrexecPolicyQualification({
    gitSha: requiredString(gitSha, 'gitSha'),
    ...
    intendedService: requiredString(intendedService, 'intendedService'),
    scenarios: Object.fromEntries(Object.entries(observations).map(([name, observation]) => [name, {
    ...
    mutationEvidence: {
      missionStoreBefore: requiredString(before?.missionStoreDigest, 'before.missionStoreDigest'),
      missionStoreAfter: requiredString(after?.missionStoreDigest, 'after.missionStoreDigest'),
      requestJournalBefore: requiredString(before?.requestJournalDigest, 'before.requestJournalDigest'),
      requestJournalAfter: requiredString(after?.requestJournalDigest, 'after.requestJournalDigest')
    }
  });
