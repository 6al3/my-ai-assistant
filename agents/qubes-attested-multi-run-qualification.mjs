...
export function evaluateAttestedMultiRunQualification(campaigns, thresholds = {}) {
  ...
  let crossRunRequestReuse = 0;
  for (const [reportIndex, report] of reports.entries()) {
    for (const requestId of report.observedRequestIds ?? []) {
      const previousOwner = requestOwners.get(requestId);
      if (previousOwner !== undefined && previousOwner !== reportIndex) crossRunRequestReuse += 1;
