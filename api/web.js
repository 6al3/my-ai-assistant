...
async function validatePublicHttpUrl(raw) {
  let url;
  try { url = raw instanceof URL ? raw : new URL(raw); } catch { throw new Error('Invalid URL.'); }
...
async function fetchWithValidatedRedirects(initialUrl, signal) {
  ...
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    const response = await fetch(current, { method: 'GET', redirect: 'manual', signal, headers: { 'User-Agent': 'DIG-GPT-Web/1.2', Accept: 'text/html,application/json,text/plain;q=0.9,*/*;q=0.5' } });
...
export default async function handler(request) {
  ...
  if (request.method !== 'GET') return json({ error: 'Only GET requests are allowed.' }, 405);
  ...
  try {
    ...
    try {
      ...
      if (length > MAX_BYTES) return json({ error: 'Remote response is too large.' }, 413);
      const buffer = await response.arrayBuffer();
      if (buffer.byteLength > MAX_BYTES) return json({ error: 'Remote response is too large.' }, 413);
      const contentType = response.headers.get('content-type') || 'application/octet-stream';
      const textLike = /^(text\/|application\/(json|xml|javascript|x-www-form-urlencoded))/i.test(contentType);
      if (!textLike) return json({ ok: response.ok, status: response.status, url: response.url, contentType, bytes: buffer.byteLength, body: null });
      const body = new TextDecoder('utf-8', { fatal: false }).decode(buffer);
      return json({ ok: response.ok, status: response.status, url: response.url, contentType, bytes: buffer.byteLength, body });
    } finally { clearTimeout(timeout); }
  } catch (error) {
    const timedOut = error?.name === 'AbortError';
    return json({ error: timedOut ? 'The remote request timed out.' : (error?.message || 'Request failed.') }, timedOut ? 504 : 400);
  }
}
